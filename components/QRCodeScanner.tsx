...
export default function QRCodeScanner({
  ...
  useEffect(() => {
    ...
    console.log('Device detection:', { isMobile: isMobileDevice, isIOS: isIOSDevice, userAgent });
  }, []);

  useEffect(() => {
    console.log('QRCodeScanner useEffect - isScanning:', isScanning, 'isInitialized:', isInitialized, 'isMobile:', isMobile);
    ...
    return () => {
      console.log('QRCodeScanner cleanup');
  ...
  }, [isScanning, isInitialized, isInitializing, isMobile]);
  ...
  const initializeScanner = async () => {
    ...
    try {
      ...
      const config = {
        fps: isMobile ? 5 : 10, // FPS plus bas sur mobile pour de meilleures performances
      ...
      scannerRef.current.render(
        ...
        (error: string) => {
          console.log('QR Scan error (non-critical):', error);
      ...
      if (error instanceof Error) {
        if (error.name === 'NotAllowedError') {
          ...
          toast.error('Accès à la caméra refusé. Veuillez autoriser l\'accès à la caméra.');
  ...
  const handleStartScan = () => {
    ...
    if (isMobile) {
      console.log('Mobile device detected, checking camera permissions...');
      
      // Test d'accès à la caméra avant d'initialiser
      navigator.mediaDevices.getUserMedia({ video: true })
        ...
        .catch((err) => {
          ...
          if (err.name === 'NotAllowedError') {
            ...
            toast.error(`Erreur d'accès à la caméra: ${err.message}`);
          }
        });
    } else {
      onStartScan();
  ...
  // Si c'est iOS, utiliser le scanner iOS spécialisé
  if (isIOS) {
    return (
      <IOSQRScanner
      ...
      {!isScanning ? (
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            {isMobile 
              ? 'Cliquez pour démarrer le scanner QR code sur mobile'
