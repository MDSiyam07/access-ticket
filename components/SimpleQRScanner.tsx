...
export default function SimpleQRScanner({
  ...
  }, [isScanning, isInitialized, isInitializing, backCameraId]);
  ...
  const initializeScanner = async () => {
    ...
    try {
      ...
      // Utilisation deviceId caméra arrière si disponible
      let cameraConfig: any;
      if (backCameraId) {
        cameraConfig = { deviceId: { exact: backCameraId } };
      } else {
        // fallback sur facingMode
  ...
  const handleStartScan = () => onStartScan();
  const handleStopScan = () => stopScanner();
      ...
      {!isScanning ? (
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            {isMobile ? 'Scanner QR code optimisé pour mobile' : 'Scanner QR code simple'}
          </p>
          {isMobile && (
          ...
          <Button onClick={handleStartScan} className="w-full h-14 text-lg festival-button">
        ...
        <div className="relative">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleStopScan} variant="outline" size="sm">Arrêter</Button>
              {onClose && (
          ...
          <div ref={containerRef} className="relative bg-gray-900 rounded-lg overflow-hidden min-h-[400px]">
            {isInitializing && (
              <div className="absolute inset-0 flex items-center justify-center bg-gray-900 z-10">
                <div className="text-center text-white">
                  <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-white mx-auto mb-4"></div>
                  <p>Initialisation de la caméra...</p>
                  {isMobile && <p className="text-sm text-gray-400 mt-2">{isIOS ? "iOS: Autorisez l'accès si demandé" : "Mobile: Autorisez l'accès si demandé"}</p>}
                </div>
              </div>
            )}
            ...
            {!isInitializing && !error && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div
                  className={`border-4 border-white border-opacity-50 rounded-lg relative ${isMobile ? 'w-48 h-48' : 'w-64 h-64'}`}
                >
          ...
          <p className="text-sm text-gray-600 mt-2 text-center">
            {isInitializing ? `Initialisation...${isMobile ? ' (Mobile)' : ''}` : 'Positionnez le QR code dans la zone de scan'}
          </p>
        </div>
      )}
    </div>
